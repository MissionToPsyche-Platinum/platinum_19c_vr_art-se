...
7. Open the Update_Artworks.ps1 file in a text editor.  Depending on your use case, you will need to change different variables.
	...
	* `bundleSource`: Change this to the path of the bundle build folder.  If you are building from Artwork, you can leave this as-is at the default `./Bundle` directory.
...
10. IF BUILDING FROM ARTWORK FOLDER: Run `.\Update_Artwork.ps1 -B` in this directory. Otherwise just running `.\Update_Artwork.ps1` with a valid Bundle folder will install the bundles to the headset.
...
Building the project causes a Bundle directory to be generated in the project root. When sending the project to others to install, this directory is vital along with the Database folder to install the artwork for the museum to display.