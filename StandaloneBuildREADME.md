...
6. Install the APK onto the headset.
7. Open the Update_Artworks.ps1 file in a text editor.  Depending on your use case, you will need to change different variables.
	...
	* `bundleSource`: Change this to the path of the bundle build folder.  If you are building from Artwork, you can leave this as-is.
	* `databasePath`: Change this to the path of the Database folder produced by running the web scraper.  **This is always necessary.**
	* `adb`: Change this to the full path of the adb.exe file, such as "C:\platform-tools\adb.exe".
...
10. Run `.\Update_Artwork.ps1 -B` in this directory.
